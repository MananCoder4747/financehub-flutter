...
class AppLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(
        child: Text('\u{20B9}', style: TextStyle(fontSize: size * 0.5, fontWeight: FontWeight.bold, color: Colors.white)),
      ),
    );
  }
}
            ...
            child: FadeTransition(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.3), blurRadius: 50, offset: const Offset(0, 25))],
                ),
                child: Column(
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [AppColors.dark, AppColors.darkSecondary]),
            ...
            Expanded(
              child: PageView(
                ...
                children: const [DashboardTab(), TransactionsTab(type: 'lend'), TransactionsTab(type: 'borrow'), PeopleTab()],
              ),
            ),
          ],
        ),
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                _NavItem(icon: Icons.dashboard_rounded, label: 'Dashboard', active: _currentIndex == 0, onTap: () { _pageController.jumpToPage(0); setState(() => _currentIndex = 0); }),
                _NavItem(icon: Icons.arrow_upward_rounded, label: 'Lent', active: _currentIndex == 1, color: AppColors.success, onTap: () { _pageController.jumpToPage(1); setState(() => _currentIndex = 1); }),
                _NavItem(icon: Icons.arrow_downward_rounded, label: 'Borrowed', active: _currentIndex == 2, color: AppColors.danger, onTap: () { _pageController.jumpToPage(2); setState(() => _currentIndex = 2); }),
                _NavItem(icon: Icons.people_rounded, label: 'People', active: _currentIndex == 3, onTap: () { _pageController.jumpToPage(3); setState(() => _currentIndex = 3); }),
              ],
            ),
          ),
        ),
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(label, style: TextStyle(fontSize: 11, fontWeight: active ? FontWeight.w600 : FontWeight.w500, color: c)),
          ],
        ),
      ),
    );
          ...
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              GridView.count(
                ...
                children: [
                  _StatCard(icon: Icons.arrow_upward_rounded, label: 'To Receive', value: '\u{20B9}${_formatAmount(lentAmount)}', color: AppColors.success, trend: Icons.trending_up),
                  _StatCard(icon: Icons.arrow_downward_rounded, label: 'To Pay', value: '\u{20B9}${_formatAmount(borrowedAmount)}', color: AppColors.danger, trend: Icons.trending_down),
                  _StatCard(icon: Icons.people_rounded, label: 'Active People', value: '${people.length}', color: AppColors.secondary),
                  _StatCard(icon: Icons.pending_actions_rounded, label: 'Pending', value: '$pendingCount', color: AppColors.warning),
                ],
              ),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                TextButton(onPressed: () {}, child: const Text('View All \u{2192}', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600))),
              ]),
              const SizedBox(height: 12),
              if (transactions.isEmpty)
                Container(padding: const EdgeInsets.all(40), alignment: Alignment.center, decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(16)),
                  child: Column(children: [
                    Icon(Icons.receipt_long_rounded, size: 48, color: AppColors.textSecondary.withOpacity(0.5)),
                    const SizedBox(height: 12),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({required this.icon, required this.label, required this.value, required this.color, this.trend});
  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(16),
          ...
          builder: (context, snapshot) {
            ...
            return Column(children: [
              ...
              Container(
                ...
                padding: const EdgeInsets.all(24),
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [AppColors.cardBg, isLend ? AppColors.success.withOpacity(0.2) : AppColors.danger.withOpacity(0.2)]),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: isLend ? AppColors.success.withOpacity(0.5) : AppColors.danger.withOpacity(0.5), width: 2),
                ),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      Icon(isLend ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded, color: isLend ? AppColors.success : AppColors.danger, size: 20),
                      const SizedBox(width: 8),
                      Text(isLend ? 'To Receive' : 'To Pay', style: const TextStyle(color: AppColors.textSecondary, fontSize: 14)),
                    ]),
                    const SizedBox(height: 8),
                  ...
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(color: (isLend ? AppColors.success : AppColors.danger).withOpacity(0.15), borderRadius: BorderRadius.circular(16)),
                    child: Icon(isLend ? Icons.savings_rounded : Icons.account_balance_rounded, color: isLend ? AppColors.success : AppColors.danger, size: 36),
                  ),
                ]),
              ),
              // List
              Expanded(
                child: transactions.isEmpty
                    ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Icon(Icons.inbox_rounded, size: 64, color: AppColors.textSecondary.withOpacity(0.3)),
                        const SizedBox(height: 16),
                        const Text('No records yet', style: TextStyle(color: AppColors.textSecondary, fontSize: 16)),
                      ]))
                    : ListView.builder(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Container(
                ...
                decoration: BoxDecoration(color: (isLend ? AppColors.success : AppColors.danger).withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
                child: Icon(isLend ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded, color: isLend ? AppColors.success : AppColors.danger),
              ),
              const SizedBox(width: 12),
              Text(isLend ? 'Add Lending' : 'Add Borrowing', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.text)),
            ]),
            const SizedBox(height: 24),
            _buildTextField(personCtrl, 'Person Name', Icons.person_rounded),
            const SizedBox(height: 12),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: isLoading ? null : () async {
                  ...
                  try {
                    ...
                    await FirebaseFirestore.instance.collection('users').doc(uid).collection('transactions').add({
                      'type': type, 'person': personCtrl.text.trim(), 'amount': amount, 'description': descCtrl.text.trim(),
                      'status': 'pending', 'settled': false, 'createdAt': FieldValue.serverTimestamp(),
                    });
                    if (context.mounted) { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Transaction added!'), backgroundColor: AppColors.success)); }
                ...
                child: isLoading ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Row(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.add_rounded), const SizedBox(width: 8), const Text('Add Transaction', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600))]),
              ),
            ),
          ]),
        );
      ...
      builder: (context, snapshot) {
        ...
        return Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            Text('${sortedPeople.length} active contacts', style: const TextStyle(color: AppColors.textSecondary)),
            const SizedBox(height: 20),
            Expanded(
              child: sortedPeople.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 16),
                      const Text('No people yet', style: TextStyle(color: AppColors.textSecondary)),
                    ]))
                  : ListView.builder(
                        ...
                        return Container(
                          ...
                          child: Row(children: [
                            Container(
                              ...
                              child: Center(child: Text(name[0].toUpperCase(), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white))),
                            ),
                            const SizedBox(width: 14),
...
class TransactionCard extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          Container(
            width: 48, height: 48,
            decoration: BoxDecoration(color: (isLend ? AppColors.success : AppColors.danger).withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: Icon(isLend ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded, color: isLend ? AppColors.success : AppColors.danger, size: 24),
          ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: (isPending ? AppColors.warning : AppColors.success).withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
              child: Text(isPending ? '\u{23F3} Pending' : '\u{2713} Settled', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: isPending ? AppColors.warning : AppColors.success)),
            ),
          ])),
        ...
        if (showActions && isPending) ...[
          ...
          Row(children: [
            Expanded(child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(foregroundColor: AppColors.success, side: const BorderSide(color: AppColors.success), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), padding: const EdgeInsets.symmetric(vertical: 12)),
            )),
            const SizedBox(width: 12),
            Expanded(child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(foregroundColor: AppColors.danger, side: const BorderSide(color: AppColors.danger), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), padding: const EdgeInsets.symmetric(vertical: 12)),
            )),
          ]),
        ],
      ]),
  ...
  Future<void> _settle(BuildContext context) async {
    final ok = await showDialog<bool>(context: context, builder: (c) => AlertDialog(
      ...
      title: const Row(children: [Icon(Icons.check_circle_rounded, color: AppColors.success), SizedBox(width: 12), Text('Settle Transaction', style: TextStyle(color: AppColors.text))]),
      content: const Text('Mark this as settled?', style: TextStyle(color: AppColors.textSecondary)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Cancel')),
        ElevatedButton(onPressed: () => Navigator.pop(c, true), style: ElevatedButton.styleFrom(backgroundColor: AppColors.success), child: const Text('Settle')),
      ],
    ));
  ...
  Future<void> _delete(BuildContext context) async {
    final ok = await showDialog<bool>(context: context, builder: (c) => AlertDialog(
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Cancel')),
        ElevatedButton(onPressed: () => Navigator.pop(c, true), style: ElevatedButton.styleFrom(backgroundColor: AppColors.danger), child: const Text('Delete')),
      ],
    ));
    if (ok == true && context.mounted) {
      final uid = FirebaseAuth.instance.currentUser!.uid;
